    ...
    if msg == "📂 浏览云盘":
        ...
        if current_url:
            ...
            if url:
                ...
                await msg_wait.edit_text("❌ 启动失败。请检查系统日志或确认 cloudflared 是否安装正确。")

    elif msg == "🔐 查看登录信息":
        url = tunnel.get_effective_public_url() or ALIST_HOST
        status = " (内网)" if "127.0.0.1" in url else " (公网)"
        await update.message.reply_text(
            f"🔐 **Alist 登录凭证**{status}\n\n🔗 地址: `{url}`\n👤 用户: `{ALIST_USER}`\n🔑 密码: `{ALIST_PASSWORD}`\n\n(如果密码不正确，请在 Termux 输入 `alist admin` 查看真实密码并修改 .env)",
            parse_mode='Markdown'
        )
        ...
        alist_str = "✅ Alist 运行中" if alist_ok else "❌ Alist 未响应 (请检查后台)"
...
async def handle_offline_download(update, url):
    ...
    if res.get('code') == 200:
        ...
        err = res.get('message')
        if "failed to add aria2 task" in str(err).lower() or "connection" in str(err).lower():
             await update.message.reply_text(f"❌ 添加失败: Alist 连接异常。\n可能原因：\n1. Alist 未启动\n2. Alist 中 Aria2 配置错误 (应为 http://127.0.0.1:6800/jsonrpc)")
        else:
